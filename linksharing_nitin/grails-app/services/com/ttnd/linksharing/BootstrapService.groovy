...
class BootstrapService {
    ...
    List<User> createUsers() {
        List<User> users = []
        if (User.count() == 0) {
            User admin = new User(email: "admin@gmail.com", username: "sysAdmin",
                    password: DefaultPassword.DEFAULT_PASSWORD, firstName: "Nitin", lastName: "Kumar Singh", admin: true,
                    active: true, confirmPassword: DefaultPassword.DEFAULT_PASSWORD)
            ...
            User user = new User(email: "normalUser@gmail.com", username: "normalUser",
                    password: DefaultPassword.DEFAULT_PASSWORD, firstName: "Nitish", lastName: "Singh", admin: false,
                    active: true, confirmPassword: DefaultPassword.DEFAULT_PASSWORD)
        ...
        users.each { user ->
            user.subscriptions.each { subscription ->
                subscription.topic.resources.each { resource ->
                    if (resource.createdBy != user && !resource.readingItems?.user?.contains(user)) {
        ...
        users.each { user ->
            user.readingItems.each { readingItem ->
                if (!readingItem.isRead) {
                    ...
                    if (resourceRating.save(flush: false)) {
